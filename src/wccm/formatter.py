...
def reverse(entry):
    ...
    if "meaning" in entry:
        if len(entry["meaning"]) >= 2:
            ...
            text+=f"{entry["meaning"][0]}\n"
    if "word" in entry and "ipa" in entry: 
        text+= f"Conlang: {entry["word"]} [{entry["ipa"]}]"
        text+= '\n'
    ...
    if "plural" in entry:
        ...
        if len(entry["plural"]) == 1:
            ...
            text+='Plurals:\n'
            for plural in enumerate(entry["plural"]):
                text+=f'- {plural["word"]} [{plural["ipa"]}]\n'
    
    if "gender" in entry: text += f'Gender: {entry["gender"]}\n'
